  ...
  this.app.Chart = (function() {
    ...
    Chart.prototype.initChart = function() {
      ...
      this.shape = new RectShape({
        fillColor: 'ORANGERED',
        width: this.canvasWidth - margin,
        height: 1,
        x: margin,
        y: this.canvasHeight
      });
      this.stage.addChild(this.shape);
      this.refLine = new RectShape({
        fillColor: 'RED',
        width: this.canvasWidth - margin,
        height: 1,
        x: margin,
      ...
      return this.stage.addChild(this.refLine);
    };

    Chart.prototype.drawChart = function(value, refValue) {
      var areaForEachTile, refY, scaleY, y;
      if (refValue == null) {
        refValue = 0;
      }
      areaForEachTile = 100;
      scaleY = value / areaForEachTile;
      y = this.canvasHeight - scaleY;
      cjs.Tween.get(this.shape).to({
        scaleY: scaleY,
        y: y
      }, 400, Ease.quartOut);
      refY = this.canvasHeight - (refValue / areaForEachTile);
      cjs.Tween.get(this.refLine).to({
        y: refY
      }, 400, Ease.quartOut);
...
(function() {
  ...
  this.app.handleListChange = function(chart1, chart2) {
    return $('input[type="radio"], input[type="checkbox"]').change(function() {
      ...
      return chart2.drawChart(sum, value);
    });
  };
