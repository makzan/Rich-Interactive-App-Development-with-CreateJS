...
(function() {
  ...
  this.app.Chart = (function() {
    function Chart(canvasId) {
      ...
      this.initChart();
      this.lastNumberOfTiles = 0;
    }

    Chart.prototype.initChart = function() {
      var chartArea, cols, i, leadingMargin, margin, shape, tileArea, tileHeight, tileWidth, x, y, _i, _ref, _results;
      this.stage.removeAllChildren();
      tileWidth = tileHeight = 10;
      margin = 5;
      leadingMargin = margin + tileWidth / 2;
      chartArea = (this.canvasWidth - margin - leadingMargin) * (this.canvasHeight - margin - leadingMargin);
      tileArea = (tileWidth + margin) * (tileHeight + margin);
      this.maxNumberOfTiles = Math.floor(chartArea / tileArea);
      cols = Math.floor((this.canvasWidth - margin) / (tileWidth + margin));
      this.shapes = [];
      _results = [];
      for (i = _i = 0, _ref = this.maxNumberOfTiles; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        ...
        _results.push(shape.scaleX = shape.scaleY = 0);
      }
      return _results;
    };

    Chart.prototype.drawChart = function(value) {
      var areaForEachTile, delay, i, numberOfTiles, _i, _ref;
      areaForEachTile = 100;
      numberOfTiles = Math.floor(value / areaForEachTile);
      for (i = _i = 0, _ref = this.maxNumberOfTiles; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        if (i < numberOfTiles) {
          delay = (i - this.lastNumberOfTiles) * 5;
          Tween.get(this.shapes[i]).wait(delay).to({
            scaleX: 1,
            scaleY: 1
          }, 400, Ease.quartOut);
        } else {
          delay = (i - this.lastNumberOfTiles) * 2;
          Tween.get(this.shapes[i]).wait(delay).to({
            scaleX: 0,
            scaleY: 0
          }, 400, Ease.quartOut);
      ...
      return this.lastNumberOfTiles = numberOfTiles;
    };
...
(function() {
  ...
  this.app.handleListChange = function(chart1, chart2) {
    $('input[type="radio"]').change(function() {
      var value;
      value = $('input[type="radio"]:checked').val();
      $('.output1').text(Math.round(value));
      return chart1.drawChart(value);
    });
