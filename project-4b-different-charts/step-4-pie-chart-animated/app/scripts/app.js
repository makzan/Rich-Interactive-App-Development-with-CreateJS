...
(function() {
  ...
  this.app.Chart = (function() {
    ...
    Chart.prototype.updateChart = function(e) {
      var endAngle, globalRotation, r, shape, startAngle, x, y;
      ...
      endAngle = this.pieData.splitDegree * Math.PI / 180 + globalRotation;
      shape = new cjs.Shape();
      shape.graphics.beginFill("GOLD").moveTo(x, y).arc(x, y, r, startAngle, endAngle).lineTo(x, y);
      this.stage.addChild(shape);
      startAngle = this.pieData.splitDegree * Math.PI / 180 + globalRotation;
    ...
    Chart.prototype.drawChart = function(leftValue, rightValue) {
      var percentage, splitDegree;
      percentage = rightValue / (leftValue + rightValue);
      splitDegree = percentage * 360;
      return Tween.get(this.pieData).to({
        splitDegree: splitDegree
      }, 400, Ease.quantOut).addEventListener('change', this.updateChart);
    };
